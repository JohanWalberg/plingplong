...
- ~~Every geometry column carries SRID 0 while the schema declares 4326.~~
  Fixed. Drizzle's `geometry` column ignored its own `srid` option twice: it
  emitted `geometry(point)` in the DDL and wrote `point(x y)` with no SRID.
  `src/db/point.ts` replaces it with a custom type that declares 4326 in both
  places, `drizzle/0005` converts the existing rows, and `src/db/point.dbtest.ts`
  guards it. `nearestListings` no longer needs its `ST_SetSRID` workaround.
