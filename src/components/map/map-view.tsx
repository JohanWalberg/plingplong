  ...
  return (
    <div className="grid h-[calc(100dvh-140px)] min-h-[520px] grid-cols-1 lg:grid-cols-[360px_1fr]">
      <ul className="hidden overflow-y-auto border-r border-line bg-surface lg:block" aria-label={tm("listLabel")}>
        {items.map((i) => (
          <li key={i.id} className={`border-b border-hairline ${i.id === selected ? "bg-primary-subtle" : ""}`}>
            <button type="button" onClick={() => setSelected(i.id)} className="flex w-full flex-col items-start gap-0.5 px-4 py-3 text-left hover:bg-bg" aria-pressed={i.id === selected}>
      ...
      <div className="relative">
        <ListingMap markers={markers} bounds={initialBounds} ariaLabel={tm("ariaLabel")} onSelect={setSelected} selectedId={selected} onMoveEnd={onMoveEnd} attribution={attribution} />
