...
import { INTENT_KEYS, intentSlug } from "@/lib/intents";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  push(() => "/", undefined, 1);
  for (const p of ["/homes", "/municipalities", "/landlords", "/how-it-works", "/coverage", "/faq", "/contact", "/about-collection", "/privacy", "/cookies", "/terms", "/for-landlords", "/safe-search"] as const) push(() => p);
  ...
  for (const m of munis) {
    ...
    for (const k of INTENT_KEYS) push((l) => ({ pathname: "/homes/[place]/[area]", params: { place: municipalitySlug(m, l), area: intentSlug(k, l) } }), undefined, 0.6);
  }
